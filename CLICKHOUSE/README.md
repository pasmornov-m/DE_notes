...
В ClickHouse существуют, так называемые агрегаторные типы данных, такие как SimpleAggregateFunction и AggregateFunction. Сейчас говорить о них смысла нет, мы затронем данную тему, когда будет расписывать работу движка AggregatingMergeTree в отдельной статье, для более продвинутого уровня.
...
Первые 2 метода не рекомендуется использовать, как минимум потому, что они не обрабатывают пустые значения **NULL** в колонке. А это иногда критично. 
...
* `TTL` - time to live, время жизни значения. В случае истечения времени, значение либо удалится, либо можно преобразовать в дефолное значение. TTL нельзя применить к полям входящие в ключ сортировки! Можно использовать только в движках MergeTree.
...
Другими словами, ни материализованную (хоть она физически существует), ни алис мы вызвать через `SELECT *` не можем, а чтобы их вызвать, необходимо в SELECT обращаться напрямую к их именам.

---

## Партицирование таблиц

Партиция - это набор данных объединённые с единым критерием, например по месяцу. Это позволяет ускорить процесс считывания данных по выбранному критерию при фильтрации.
...
Важно понимать, что при разделении таблиц по партициям, данные сливаются в рамках своих партиций. Из-за этого случаются небольшие коллизии в данных на определённых движках. (Об этом мы поговорим в стать о движках)
...
Данный индекс работает достаточно просто, тк берутся определенные гарнулы, заданные с помощью параметра `GRANULARITY` и в данном куске ищется минимальное и максимальное значение.
...
Как упоминалось ранее, существуют 2 дополнительных типа данных **SimpleAggregateFunction** и **AggregateFunction**.
...
А теперь посмотрим на данные, но их невозможно прочитать. Чтобы достать необходимые для нас данные необходимо воспользоваться коомбинатом **-Merge**, вот так:
