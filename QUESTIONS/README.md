...
* [Дистрибуция что это, какие типы есть, как определить? что таблица дистрибуцирована нормально](#дистрибуция-что-это-какие-типы-есть-как-определить-что-таблица-дистрибуцирована-нормально)
* [Таблицы дистрибуцированы по разным ключам, как избежать редистрибьют моушена](#таблицы-дистрибуцированы-по-разным-ключам-как-избежать-редистрибьют-моушена)
...
* [Проблемы DEFAULT PARTITION](#проблемы-default-partition)
* [Что такое субпартиции](#что-такое-субпартиции)
...
* [Как получить настоящее текущее время](#как-получить-настоящее-текущее-время)
* [Как извлечь дату, время, месяц из поля с датой](#как-извлечь-дату-время-месяц-из-поля-с-датой)
* [Что происходит при добавлении новой записи на уровне файлов в PostgreSQL](#что-происходит-при-добавлении-новой-записи-на-уровне-файлов-в-postgresql)
...
* [Чем отличаются генератор и итератор](#чем-отличаются-генератор-и-итератор)
* [Почему нельзя использовать изменяемые значения как аргументы по умолчанию?](#почему-нельзя-использовать-изменяемые-значения-как-аргументы-по-умолчанию)
* [Почему все лямбды в цикле используют одно и то же значение?](#почему-все-лямбды-в-цикле-используют-одно-и-то-же-значение)
...
## Дистрибуция что это, какие типы есть, как определить? что таблица дистрибуцирована нормально

**Дистрибуция** — это способ распределения строк таблицы между `primary`-сегментами Greenplum.

В отличие от обычного PostgreSQL, где таблица физически хранится на одном сервере, в Greenplum данные таблицы распределяются по сегментам, чтобы разные сегменты могли **параллельно** обрабатывать разные части данных.

Например, если в кластере 4 primary-сегмента:

```text
Таблица orders
        │
        ├── Segment 1 → часть строк
        ├── Segment 2 → часть строк
        ├── Segment 3 → часть строк
        └── Segment 4 → часть строк
```

Цель хорошей дистрибуции — чтобы данные распределялись **примерно равномерно**, а часто соединяемые таблицы по возможности были распределены совместимо.
...
Строка отправляется на сегмент на основании хеша значения указанного столбца или набора столбцов.
...
Для каждой строки Greenplum вычисляет hash от `user_id` и на основании этого определяет сегмент.

Например:

```text
user_id = 101 → Segment 2
user_id = 102 → Segment 4
user_id = 103 → Segment 1
...
```

Если указано несколько столбцов:

```sql
DISTRIBUTED BY (user_id, product_id)
```

учитывается комбинация их значений.

Это основной способ управляемой дистрибуции.

---

#### 2. `DISTRIBUTED RANDOMLY`

Строки распределяются между сегментами без использования определённого ключа.
...
Такой вариант может обеспечить достаточно равномерное распределение, если нет подходящего distribution key.

Однако есть недостаток: таблицу нельзя естественным образом **колокализовать** с другой таблицей по ключу JOIN.
...
Для небольшой таблицы это может быть удобно:

```text
Segment 1 → вся countries
Segment 2 → вся countries
Segment 3 → вся countries
Segment 4 → вся countries
```

Тогда при JOIN каждый сегмент уже имеет локальную копию справочника и дополнительный обмен данными может не потребоваться.

Использовать такой вариант следует в основном для **небольших справочных таблиц**, потому что размер данных фактически умножается на количество сегментов.

---

### Что такое хороший distribution key

При выборе `DISTRIBUTED BY` обычно преследуются две цели:

1. **равномерно распределить данные между сегментами**;
2. **избегать лишнего Motion при JOIN**.

Например, есть две большие таблицы:
...
В хорошем случае данные с одинаковым `user_id` уже находятся на одном сегменте. Поэтому Greenplum может выполнить JOIN локально, без перераспределения больших объёмов данных.

Если же:

```text
orders   → DISTRIBUTED BY (user_id)
payments → DISTRIBUTED BY (order_id)
```

то для JOIN по `user_id` может потребоваться перераспределение данных между сегментами — `REDISTRIBUTE MOTION`.

---

### Как определить, что таблица распределена нормально

Нужно проверить прежде всего **distribution skew** — насколько неравномерно данные распределены между сегментами.

Самый простой способ — посмотреть количество строк на каждом сегменте:
...
Это хорошая дистрибуция: объём данных примерно одинаковый.
...
В этом случае один сегмент будет обрабатывать намного больше данных, чем остальные.

---

#### Проверка через `gp_toolkit`

Для диагностики распределения можно использовать системные представления и функции Greenplum из `gp_toolkit`.

Например, полезно посмотреть информацию о размере таблиц на сегментах:

```sql
SELECT *
FROM gp_toolkit.gp_skew_coefficients
WHERE sktablename = 'table_name';
```
...
Они помогают оценить степень перекоса данных.

На практике конкретный набор доступных представлений может зависеть от версии Greenplum, поэтому для точной диагностики часто используют и прямую агрегацию по `gp_segment_id`.

---

### Как количественно оценивать skew

Обычно смотрят не только на сам факт неравномерности, но и на то, насколько сильно отличается самый загруженный сегмент.

Простейшая оценка:

```text
max(rows_per_segment) / avg(rows_per_segment)
```
...
означает серьёзный перекос.

Также полезно смотреть на **размер данных**, а не только на количество строк:
...
Потому что одинаковое количество строк ещё не гарантирует одинаковый объём данных, если строки сильно различаются по размеру.

---

### Откуда появляется skew

Чаще всего причина — плохой distribution key.
...
Количество различных значений (`cardinality`) маленькое, поэтому hash распределение не сможет эффективно разбросать строки.

Особенно опасны:

* поля с малым количеством уникальных значений;
* поля с большим количеством `NULL`;
* сильно несбалансированные значения;
* ключи, по которым данные естественно сильно концентрируются.

---

### Что ещё нужно проверить

Хорошая дистрибуция — это не только равномерное количество строк.

Также нужно посмотреть на запросы и планы:
...
Если таблицы часто JOIN-ятся по `user_id`, имеет смысл рассмотреть:

```text
table A → DISTRIBUTED BY (user_id)
table B → DISTRIBUTED BY (user_id)
```

Тогда можно уменьшить количество `Motion`.
...
Если большие таблицы постоянно перераспределяются при JOIN, distribution keys, возможно, выбраны неудачно.

---

### Как ответить на собеседовании

> **Дистрибуция в Greenplum — это способ распределения строк таблицы между primary-сегментами. Основные варианты — `DISTRIBUTED BY`, `DISTRIBUTED RANDOMLY` и `DISTRIBUTED REPLICATED`.**
>
> **При `DISTRIBUTED BY` сегмент определяется по хешу distribution key. При `DISTRIBUTED RANDOMLY` строки распределяются без заданного ключа, а при `DISTRIBUTED REPLICATED` полная копия таблицы хранится на каждом сегменте.**
>
> **Нормальная дистрибуция должна быть равномерной — без data skew. Я могу проверить это, например, через `gp_segment_id`, посчитав количество строк на каждом сегменте, а также посмотреть skew-метрики из `gp_toolkit`. Кроме того, смотрю планы `EXPLAIN`: если при частых JOIN между большими таблицами возникает `REDISTRIBUTE MOTION`, это может быть признаком неудачного выбора distribution key.**
>
> **При выборе ключа стараюсь одновременно обеспечить равномерность распределения и колокацию таблиц по часто используемым ключам JOIN.**

---

## Таблицы дистрибуцированы по разным ключам, как избежать редистрибьют моушена

Если две большие таблицы **дистрибуцированы по разным ключам**, то при `JOIN` по другому ключу Greenplum обычно вынужден выполнить `REDISTRIBUTE MOTION`. Полностью «запретить» его нельзя — нужно изменить способ выполнения запроса или физическую организацию данных.

### Основные варианты

#### 1. Перераспределить одну из таблиц заранее

Если таблица часто участвует в JOIN по определённому ключу, лучше физически распределить её по этому ключу:
...
```text
table_a → DISTRIBUTED BY (join_key)
table_b → DISTRIBUTED BY (join_key)
```
...
может выполняться локально на сегментах без `REDISTRIBUTE MOTION`.

Это **лучший вариант**, если таблицы большие и такой JOIN выполняется регулярно.

---

#### 2. Использовать `BROADCAST MOTION` для маленькой таблицы

Если одна таблица небольшая, её можно целиком отправить на каждый сегмент.

Тогда вместо перераспределения обеих таблиц Greenplum может выбрать broadcast:

```text
Большая таблица
    ↓
остаётся на сегментах

Маленькая таблица
    ↓
копируется на все сегменты
```

Это особенно эффективно для небольших справочников.
...
Для такого JOIN broadcast обычно существенно дешевле, чем redistribution 1 TB.

При этом `BROADCAST MOTION` для больших таблиц — плохой сценарий.

---

#### 3. Перераспределить только одну сторону JOIN

Необязательно перераспределять обе таблицы. Greenplum может выполнить:

```text
table A → остаётся как есть
table B → Redistribute Motion по join_key
```

Это выгоднее, чем двигать обе таблицы.

Но важно понимать: если `table B` огромная, `REDISTRIBUTE MOTION` всё равно может быть дорогим.

---

#### 4. Изменить порядок или структуру запроса

Иногда `REDISTRIBUTE MOTION` появляется из-за того, как написан запрос.

Например, сначала можно:

* отфильтровать данные;
* агрегировать;
* уменьшить количество столбцов;
* убрать ненужные строки;

а уже потом выполнять JOIN.
...
WHERE b.event_date >= DATE '2026-01-01';
```

часто выгоднее сначала уменьшить `large_b`:
...
Тогда Greenplum перераспределяет уже меньший объём данных.

---

#### 5. Создать промежуточную таблицу с нужной дистрибуцией

Очень распространённый практический подход:
...
Если `table_a` уже распределена по `user_id`, JOIN становится колокализованным.

Это особенно полезно, если исходную таблицу менять нельзя, но в рамках конкретного ETL-процесса нужен эффективный JOIN.

---

#### 6. Выбрать правильный distribution key на этапе проектирования

Это самый важный вариант.

При выборе `DISTRIBUTED BY` нужно учитывать **не только равномерность распределения**, но и наиболее частые JOIN.
...
Так несколько больших таблиц можно соединять по `user_id` локально.

Но нельзя жертвовать равномерностью ради колокации:

```sql
DISTRIBUTED BY (status)
```

может полностью убить производительность из-за skew, даже если `status` используется в JOIN.

---

### Что делать, если таблицы уже физически дистрибуцированы по разным ключам?

На практике порядок действий такой:

```text
1. Посмотреть EXPLAIN
        ↓
2. Найти REDISTRIBUTE MOTION
        ↓
3. Определить размер обеих таблиц
        ↓
4. Если одна маленькая → BROADCAST
        ↓
5. Если обе большие → думать о колокации
        ↓
6. Если исходную таблицу менять нельзя →
   промежуточная таблица с нужным DISTRIBUTED BY
```
...
Если обе таблицы большие, оптимальный долгосрочный вариант — изменить распределение так, чтобы обе были колокализованы по `customer_id`:
...
Но если `orders` очень большая и `customer_id` распределяется плохо, такой выбор может привести к skew. Поэтому **сам факт отсутствия `REDISTRIBUTE MOTION` ещё не означает хороший дизайн**.

### Что сказать на собеседовании

> «Если таблицы дистрибуцированы по разным ключам, JOIN по другому ключу обычно вызывает `REDISTRIBUTE MOTION`. Чтобы его избежать, нужно либо сделать таблицы колокализованными по ключу JOIN, либо использовать `BROADCAST`, если одна из таблиц маленькая. Если физическую дистрибуцию менять нельзя, можно предварительно создать промежуточную таблицу с нужным `DISTRIBUTED BY`. При этом я всегда смотрю на `EXPLAIN`, размер таблиц и skew, потому что устранить Motion ценой сильного перекоса — тоже плохая оптимизация».
...
## Проблемы DEFAULT PARTITION

`DEFAULT PARTITION` — это партиция, в которую попадают строки, не соответствующие ни одной явно заданной партиции. На первый взгляд это удобно, но при проектировании и эксплуатации есть несколько важных проблем.

### 1. `DEFAULT` может скрывать ошибочные данные

Например, таблица партиционирована по дате:
...
Если пришла строка с датой `2026-04-15`, она попадёт в `orders_default`.

Это может скрыть ошибку в ETL: вместо того чтобы сразу получить ошибку из-за отсутствующей партиции, данные будут успешно загружены, но окажутся не там, где ожидалось.

---

### 2. `DEFAULT` практически всегда участвует в проверках и может ухудшать `partition elimination`

Если запрос не позволяет однозначно исключить `DEFAULT`, она также может потребоваться для проверки.
...
оптимизатор может быть вынужден учитывать `DEFAULT`, поскольку заранее неизвестно, какие значения там находятся.

Поэтому наличие большого количества данных в `DEFAULT` может уменьшить эффективность partition pruning / elimination.

---

### 3. При добавлении новой партиции `DEFAULT` может потребоваться очистить

Предположим, сейчас есть:
...
Если в `orders_default` уже находятся строки за март, новая партиция конфликтует с ними.

Поэтому перед добавлением новой партиции необходимо обеспечить, чтобы `DEFAULT` не содержала строки, относящиеся к новому диапазону.
...
Если в `orders_default` есть соответствующие строки, операция может завершиться ошибкой.

---

### 4. `DEFAULT` может сильно вырасти

Если новые партиции создаются несвоевременно, все новые данные будут постепенно накапливаться в `DEFAULT`.
...
Тогда `DEFAULT` превращается фактически в ещё одну большую партицию, которая сама перестаёт быть «защитным» механизмом и становится проблемой производительности и обслуживания.

---

### 5. Сложнее контролировать качество данных

`DEFAULT` стоит воспринимать не как обычное место хранения данных, а скорее как **буфер для неожиданных значений**.

На практике полезно периодически проверять её:
...
Если в ней постоянно появляются данные, это сигнал, что необходимо:

* создавать новые партиции заранее;
* исправить логику загрузки;
* проверить значения ключа партиционирования.

### Как ответить на собеседовании

> `DEFAULT PARTITION` удобна тем, что принимает строки, которые не попадают ни в одну явно заданную партицию. Но её основная проблема — она может скрывать ошибки в загрузке данных. Кроме того, при большом объёме данных в DEFAULT ухудшается partition elimination, а при добавлении новой партиции приходится проверять, нет ли в DEFAULT строк, которые должны туда попасть. Поэтому DEFAULT обычно используют как защитный механизм и регулярно контролируют её содержимое.

---

## Что такое субпартиции

**Субпартиционирование** — это разбиение уже существующей партиции на дополнительные партиции, то есть создание **нескольких уровней партиционирования**.

Проще говоря, сначала таблица делится на крупные части, а затем каждая такая часть дополнительно разбивается на более мелкие.

Например, таблицу продаж можно сначала разделить по году:
...
а внутри `sales_2026` сделать дополнительное разбиение по месяцам:
...
Таблица
   ↓
Партиции первого уровня
   ↓
Субпартиции второго уровня
```

### Зачем нужны субпартиции

Основная причина — удобнее работать с очень большими таблицами.
...
может отфильтровать сначала весь 2026 год, а затем только мартовскую субпартицию.

Это позволяет уменьшить объём данных, который необходимо просканировать.

### Важный момент

Партиция верхнего уровня сама может быть **partitioned table**, то есть не содержать конечные строки напрямую, а состоять из субпартиций.

В PostgreSQL, например, можно использовать разные типы партиционирования на разных уровнях:
...
> **Субпартиции — это второй и последующие уровни партиционирования. Партиция сама становится partitioned table и дополнительно делится на более мелкие части. Например, таблицу можно сначала разбить по году, а внутри каждого года — по месяцам. Это удобно для очень больших таблиц и позволяет более точно применять partition pruning.**
...
## Как получить настоящее текущее время

Здесь есть важный нюанс: обычные `now()` и `CURRENT_TIMESTAMP` в Greenplum, как и в PostgreSQL, возвращают **время начала текущей транзакции**, а не фактическое время в момент вызова. Поэтому внутри одной длительной транзакции значение `now()` не меняется.

Если нужно получить **реальное текущее время на момент выполнения функции**, используется:

```sql
SELECT clock_timestamp();
```

`clock_timestamp()` возвращает фактическое системное время и может изменяться даже в рамках одного SQL-запроса.
...
Оба вызова `now()` вернут практически одно и то же время, потому что это время начала транзакции.
...
вернёт два разных значения — второе будет примерно на 5 секунд позже.
...
| `CURRENT_TIMESTAMP`       | время начала транзакции           |
| `transaction_timestamp()` | время начала транзакции           |
| `statement_timestamp()`   | время начала текущего SQL-запроса |
| `clock_timestamp()`       | фактическое текущее время         |

Для ответа на собеседовании:

> **Если нужно получить настоящее текущее время в Greenplum, использую `clock_timestamp()`. `now()` и `CURRENT_TIMESTAMP` возвращают время начала транзакции и поэтому могут не меняться в течение длительной транзакции.**

---

## Как извлечь дату, время, месяц из поля с датой

Если в Greenplum/PostgreSQL есть поле типа `DATE` или `TIMESTAMP`, отдельные части даты можно получить несколькими способами.
...
SELECT
    ...
    EXTRACT(MINUTE FROM event_date) AS minute,
    EXTRACT(SECOND FROM event_date) AS second
FROM events;
```
...
Это практически аналог `EXTRACT`:
...
### Если нужен именно месяц как дата

Например, привести любую дату к первому дню месяца:

```sql
SELECT DATE_TRUNC('month', event_date)
FROM events;
```
...
Это особенно удобно для группировки:
...
> Для извлечения части даты использую `EXTRACT` или `DATE_PART`, например `EXTRACT(MONTH FROM dt)`. Если нужно привести дату к началу месяца, года или дня, использую `DATE_TRUNC()`.
...
## Что происходит при добавлении новой записи на уровне файлов в PostgreSQL

Если говорить именно про **PostgreSQL на уровне файловой системы**, то при `INSERT` происходит не «добавление записи в файл» в привычном смысле. PostgreSQL работает со **страницами (pages)**, WAL и отдельными файлами таблиц и индексов.
...
INSERT
  ↓
буфер PostgreSQL (shared_buffers)
  ↓
изменение страницы таблицы в памяти
  ↓
запись WAL
  ↓
COMMIT
  ↓
WAL гарантированно записан на диск
  ↓
позже dirty page таблицы записывается на диск
```

#### 1. PostgreSQL находит таблицу

У таблицы есть физическое представление в `PGDATA/base/...`.

Например, таблица имеет некоторый `relfilenode` — имя основного физического файла.
...
└── base/
    └── 16384/
        ├── 24576       ← данные таблицы
        ├── 24576_fsm   ← Free Space Map
        └── 24576_vm    ← Visibility Map
```

Файл данных разбит на страницы, обычно по **8 KB**.
...
#### 2. Строка сначала изменяет страницу в памяти

PostgreSQL старается не писать каждую вставленную строку сразу на диск.

Нужная страница загружается в **shared buffers**:
...
После `INSERT` страница становится **dirty page** — изменённой, но ещё не записанной обратно на диск.

То есть после самой вставки данные могут некоторое время находиться только в памяти.

---

#### 3. PostgreSQL записывает WAL

Одновременно PostgreSQL формирует запись в **WAL — Write-Ahead Log**.

Файлы WAL находятся в:

```text
$PGDATA/pg_wal/
```

WAL нужен для **crash recovery**.

Главный принцип:

> Сначала информация об изменении должна попасть в WAL, и только потом изменённые страницы таблиц могут быть записаны на диск.

Поэтому WAL называется **Write-Ahead**.
...
PostgreSQL гарантирует, что соответствующая WAL-информация записана в устойчивое хранилище согласно настройкам надёжности.

При этом **сама страница таблицы не обязана уже находиться на диске**.
...
shared_buffers:
┌─────────────────────┐
│ page с новой строкой│ ← dirty
└─────────────────────┘

pg_wal:
┌─────────────────────┐
│ WAL record          │ ← уже на диске
└─────────────────────┘
...
Если сервер внезапно упадёт, PostgreSQL при восстановлении прочитает WAL и восстановит изменения в страницах таблиц.

---

#### 5. Когда записывается сама таблица

Позже background writer / checkpointer записывают dirty pages из `shared_buffers` в файлы таблиц.
...
позже
  ↓
dirty page → table file
```

Поэтому **момент `COMMIT` и момент записи страницы таблицы на диск — не одно и то же**.

---

### Что происходит с индексами

Если у таблицы есть индекс:

```sql
CREATE INDEX idx_users_id ON users(id);
```

то при `INSERT` PostgreSQL должен обновить **и таблицу, и индекс**.
...
Изменения обеих структур также отражаются в WAL.

Поэтому индексы увеличивают стоимость `INSERT`:

```text
нет индекса
→ изменение только таблицы
...
Именно поэтому большое количество индексов может заметно замедлять запись.

---

### Что находится внутри страницы

Для обычной **heap table** страница содержит:
...
Строка PostgreSQL называется **tuple**.

При вставке PostgreSQL помещает новую tuple в подходящую страницу, где достаточно свободного места.

Для поиска подходящей страницы используется, в частности, **Free Space Map (FSM)**.

---

### А если INSERT большой?
...
PostgreSQL будет многократно изменять страницы.
...
WAL при этом обычно сильно увеличивается.

---

### Важный момент: `INSERT` не обязательно создаёт новый физический файл

Если вы добавили одну строку:

```sql
INSERT INTO users ...
```

PostgreSQL обычно **не создаёт новый файл**.

Он изменяет существующий relation file и его страницы.

Если таблица перерастает определённый размер, PostgreSQL автоматически создаёт дополнительные сегменты файла.

Обычно физические файлы relation имеют ограниченный размер, после чего появляются:
...
### А что происходит с MVCC?

При `INSERT` PostgreSQL создаёт **новую версию строки** и записывает информацию о транзакции, которая её создала (`xmin`).
...
`xmin = 12345` означает, что строку создала транзакция `12345`.

`xmax = 0` означает, что она пока не была удалена или заменена другой версией.

---

### Полная схема

Для собеседования удобно запомнить:

```text
                 INSERT
                   ↓
            найти свободную page
                   ↓
         изменить page в RAM
                   ↓
              WAL record
...
> При `INSERT` PostgreSQL не обязательно сразу пишет строку в файл таблицы. Сначала изменяется соответствующая страница в `shared_buffers`, после чего информация об изменении записывается в WAL. При `COMMIT` WAL должен быть сохранён на диске, что обеспечивает durability. Сама dirty page таблицы может быть записана на диск позже background writer или checkpoint. Если у таблицы есть индексы, они также обновляются и их изменения попадают в WAL.

**Ключевая мысль:** `COMMIT` гарантирует сохранность изменения через WAL, а не означает, что сама страница таблицы уже записана в файл данных.
...
## Чем отличаются генератор и итератор

Различие между **итератором** и **генератором** — это различие между **интерфейсом (протоколом)** и **конкретным механизмом его реализации**.

---

### Итератор (Iterator)

**Итератор** — это любой объект, который реализует протокол итерации:
...
Итератор = объект, из которого можно последовательно извлекать значения.
...
**Генератор** — это специальный тип итератора, который создаётся автоматически:

* через функцию с `yield`
* или generator expression

Python сам реализует `__iter__` и `__next__`.
...
| --------------------- | ------------------------------- | ------------------------------- |
| Что это               | Протокол / интерфейс            | Конкретная реализация итератора |
| Как создаётся         | Класс с `__iter__` и `__next__` | Функция с `yield`               |
| Объём кода            | Большой (ручная реализация)     | Минимальный                     |
| Управление состоянием | Вручную                         | Автоматически                   |
| Память                | Зависит от реализации           | Ленивые вычисления (lazy)       |
| Тип объекта           | Любой класс                     | `generator`                     |

---

### Ключевая концептуальная разница

**Каждый генератор — это итератор.**
**Но не каждый итератор — генератор.**

Генератор = синтаксический сахар + state machine, которую Python генерирует автоматически.

---

## Почему нельзя использовать изменяемые значения как аргументы по умолчанию?


**Проблема:** изменяемые аргументы по умолчанию (`list`, `dict`, `set`) создаются **один раз при определении функции**, а не при каждом вызове.

```python
def f(x, arr=[]):   # плохая практика
    arr.append(x)
    return arr
```

Один и тот же объект `arr` используется во всех вызовах → общее состояние → скрытые баги и утечки данных (особенно в long-running сервисах).

**Почему так:** `def` — исполняемая инструкция; значения default-аргументов вычисляются во время загрузки модуля и лежат в `__defaults__`.
...
Новый объект создаётся во время выполнения функции (runtime), а не при её определении.

**Где ещё встречается:** в декораторах — всё, что объявлено вне `wrapper`, выполняется один раз при импорте и становится общим состоянием.

---

## Почему все лямбды в цикле используют одно и то же значение?

**Проблема:** замыкания в Python используют **позднее связывание (late binding)**.
Лямбда или вложенная функция **не запоминает значение переменной**, а хранит ссылку на неё и берёт значение **в момент вызова**, а не создания.
...
Все функции видят одно и то же `i = 4` (последнее значение цикла).

**Почему:** цикл уже завершён, а переменная `i` одна и та же для всех замыканий.

**Правильный способ (зафиксировать значение):**

```python
multipliers = [lambda x, i=i: x * i for i in range(5)]
```

Default-аргументы вычисляются **в момент создания функции**, поэтому значение `i` «замораживается».

**Где критично:** обработчики событий, callbacks, асинхронный код, UI — все обработчики могут начать вести себя как «последний».
