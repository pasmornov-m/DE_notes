...
* Некорректное завершение транзакций в NameNode может замедлить работу.

---

### 7. Безопасность и доступ

* Неадекватная настройка прав доступа может привести к несанкционированному доступу к данным.
...
Hadoop — это не только HDFS и MapReduce, а целая экосистема инструментов для хранения, управления и анализа больших данных. Основная идея заключается в том, чтобы эффективно и надёжно обрабатывать распределённые наборы данных, масштабируясь по мере необходимости.
...
7. **Колонковые форматы и фильтрация по партициям**: Parquet/ORC + partition pruning + predicate pushdown.
...
* **Atomicity (атомарность):** Все операции в транзакции либо выполнены полностью, либо откатаны при ошибке.
